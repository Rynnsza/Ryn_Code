...
<html lang="id" class="scroll-smooth">
  <head>
    ...
    <title>@yield('title', $setting->name ?? 'RynCode')</title>
    <meta name="description" content="{{ ($setting->name ?? 'RynCode') . ' - Solusi desa digital' }}" />
  ...
  <body
    ...
    x-init="
      ...
      const sectionIds = ['hero', 'why-ryncode', 'services', 'implementation-timeline', 'portfolio', 'testimonials', 'contact'];
  ...
  >
    <header
      class="z-20 sticky top-0 border-b border-slate-200 bg-white dark:border-slate-800 dark:bg-slate-950"
    >
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-3 flex items-center justify-between gap-3">
        <div class="flex items-center gap-2">
          <div class="h-9 w-9 rounded-xl bg-emerald-500/10 border border-emerald-400/40 flex items-center justify-center overflow-hidden">
            ...
            @else
              <span class="text-lg font-semibold tracking-tight text-emerald-300">RC</span>
            @endif
          </div>
          <div>
            <div class="text-sm font-semibold tracking-tight">{{ $setting->name ?? 'RynCode' }}</div>
    ...
    <footer
      class="mt-16 border-t border-slate-200 bg-slate-50 dark:border-slate-800 dark:bg-slate-950"
    >
      <div class="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8 text-xs text-slate-500 dark:text-slate-300 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
        <div>
          <div class="font-semibold text-slate-800 dark:text-slate-200">{{ $setting->name ?? 'RynCode' }}</div>
          <div class="text-[11px]">Mengakar di desa, bertumbuh bersama sekolah dan UMKM.</div>
        </div>
        <div class="flex flex-wrap gap-4">
          <div>© {{ date('Y') }} {{ $setting->name ?? 'RynCode' }}. All rights reserved.</div>
        </div>
      </div>
    </footer>
